...
impl<'a> TypeDefRow<'a> {
    fn name(&self) -> std::io::Result<&'a str> {
        self.db.strings(self.db.cell32(&self.db.type_def, self.index, 1)?)
    }
    fn namespace(&self) -> std::io::Result<&'a str> {
        self.db.strings(self.db.cell32(&self.db.type_def, self.index, 2)?)
    }
}
